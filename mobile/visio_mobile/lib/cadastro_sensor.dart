...
class _SensoresAdminPageState extends State<SensoresAdminPage> {
  ...
  void _abrirModal({Map<String, dynamic>? sensor}) {
    ...
    final nomeCtrl = TextEditingController(text: sensor?['nome'] ?? '');
    final tipoCtrl = TextEditingController(text: sensor?['tipo'] ?? '');
    final salaCtrl = TextEditingController(text: sensor?['sala'] ?? '');
    final tensaoCtrl = TextEditingController(text: sensor?['tensao'] ?? '');
    final macCtrl = TextEditingController(text: sensor?['mac'] ?? '');
    String status = sensor?['status'] ?? 'ativo';
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                _modalField(macCtrl, 'MAC', Icons.cable_outlined),

                const SizedBox(height: 14),

                Row(
                  children: ['ativo', 'inativo'].map((s) {
